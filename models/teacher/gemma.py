...
from transformers.models.gemma.modeling_gemma import is_torch_fx_available, add_start_docstrings_to_model_forward, GEMMA_INPUTS_DOCSTRING, Optional, List, BaseModelOutputWithPast, Union, Tuple, GemmaPreTrainedModel, SequenceClassifierOutputWithPast, GemmaModel
from train_dcidsfpos import logger
import types

'''
Implementation for unmask-gemma, doesn't support flashattention. transformers version is 4.39.3
...
if is_torch_fx_available():
    ...
    _prepare_4d_attention_mask = torch.fx.wrap(transformers.modeling_attn_mask_utils._prepare_4d_attention_mask)
...
class GemmaForDCIDSFPOS(GemmaPreTrainedModel):
...
class Gemma(nn.Module):
    ...
    def __init__(self, name='google/gemma-7b',head={'type':'sf','nclasses':2,'pooling':'last'},loracfg={'lora_r':64,'lora_alpha':16,'lora_dropout':0.1},freeze = True,load_ft=None,mymask=True) :
        ...
        # use the same target_modules as llama default cfg in peft
        if self.head_cfg['type'] in ('dc','id','cdc'):
            self.peft_config = LoraConfig(task_type=TaskType.SEQ_CLS, inference_mode=False, r=self.loracfg['lora_r'], lora_alpha=self.loracfg['lora_alpha'], lora_dropout=self.loracfg['lora_dropout'],target_modules=["q_proj", "v_proj"])
        else:
            self.peft_config = LoraConfig(task_type=TaskType.TOKEN_CLS, inference_mode=False, r=self.loracfg['lora_r'], lora_alpha=self.loracfg['lora_alpha'], lora_dropout=self.loracfg['lora_dropout'],target_modules=["q_proj", "v_proj"])
        ...
        try:
            self.model = GemmaForDCIDSFPOS.from_pretrained('{}/{}'.format(self.save_path,self.model_name),pad_token_id=self.tokenizer.pad_token_id,**mykwargs).bfloat16()
        except:
            self.model = GemmaForDCIDSFPOS.from_pretrained(self.model_name, token=' ',pad_token_id=self.tokenizer.pad_token_id,**mykwargs).bfloat16()
            self.model.save_pretrained('{}/{}'.format(self.save_path,self.model_name), from_pt=True)

        # self.model = GemmaForDCIDSFPOS(self.save_path, name=self.model_name,head=self.head_cfg,freeze=self.freeze)
